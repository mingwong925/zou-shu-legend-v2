...
export interface GameStatePayload { coin: { x: number; y: number; dir: string; lives: number }; cacti: { id: string; x: number; y: number; dir: string; color: string; playerIndex: number }[]; dots: string[]; goodShows: { x: number; y: number }[]; fright: number; winner: "coin" | "cacti" | null; }
