...
function move(entity: { x: number; y: number; dir: Dir }, speed: number): void { if (entity.dir === "none") return; const [dx, dy] = dirs[entity.dir]; const distance = speed / 20; const steps = Math.max(1, Math.ceil(distance / 2)); const step = distance / steps; for (let i = 0; i < steps; i++) { const x = entity.x + dx * step, y = entity.y + dy * step; if (!walkable(x, y)) { entity.dir = "none"; return; } entity.x = x; entity.y = y; } }
...
function tick(room: Room): void { if (room.winner) return; const now = Date.now(); if (now >= room.nextGoodShowAt) spawnGoodShow(room); room.coin.dir = room.inputs.get("coin") ?? room.coin.dir; move(room.coin, COIN_SPEED); for (const player of room.players.values()) if (player.role === "cactus" && player.jailedUntil <= now) { if (player.x === 240 && player.y === 336) { player.x = 272; player.y = 240; } player.dir = room.inputs.get(player.id) ?? player.dir; move(player, CACTUS_SPEED); }
