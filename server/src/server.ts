...
type Room = { code: string; players: Map<string, Player>; inputs: Map<string, Dir>; dots: Set<string>; coin: { x: number; y: number; dir: Dir; lives: number }; goodShows: { x: number; y: number }[]; nextGoodShowAt: number; frightUntil: number; winner: Winner; timer?: NodeJS.Timeout };
...
function newRoom(code: string): Room { return { code, players: new Map(), inputs: new Map(), dots: dots(), coin: { x: 112, y: 624, dir: "none", lives: 3 }, goodShows: [], nextGoodShowAt: Date.now() + GOOD_SHOW_INTERVAL, frightUntil: 0, winner: null }; }
...
function snapshot(room: Room) { return { coin: room.coin, cacti: [...room.players.values()].filter((p) => p.role === "cactus").map((p) => ({ id: p.id, x: p.x, y: p.y, dir: p.dir, color: p.color, playerIndex: p.playerIndex })), dots: [...room.dots], goodShows: room.goodShows, fright: Math.max(0, room.frightUntil - Date.now()), winner: room.winner }; }
