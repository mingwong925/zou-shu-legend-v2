...
function tick(room: Room): void { if (room.winner) return; const now = Date.now(); if (now >= room.nextGoodShowAt) spawnGoodShow(room); room.coin.dir = room.inputs.get("coin") ?? room.coin.dir; move(room.coin, COIN_SPEED, 10, 10); for (const player of room.players.values()) if (player.role === "cactus" && player.jailedUntil <= now) { if (player.x === 240 && player.y === 336) { player.x = 272; player.y = 240; } player.dir = room.inputs.get(player.id) ?? player.dir; move(player, CACTUS_SPEED, 10, 13); }
