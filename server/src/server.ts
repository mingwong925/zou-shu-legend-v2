...
const TURN_GRACE_DISTANCE = 14;
const TURN_BUFFER_MS = 700;
...
type BufferedInput = { dir: Dir; at: number };
...
type Room = { code: string; players: Map<string, Player>; inputs: Map<string, BufferedInput>; dots: Set<string>; coin: { x: number; y: number; dir: Dir; want: Dir; lives: number }; goodShows: { x: number; y: number }[]; nextGoodShowAt: number; frightUntil: number; message: string | null; messageUntil: number; winner: Winner; timer?: NodeJS.Timeout };
...
function applyQueuedTurn(entity: { x: number; y: number; dir: Dir; want: Dir }, turnTolerance: number): void { if (entity.want === "none" || entity.want === entity.dir) return; const tileX = Math.floor(entity.x / TILE), tileY = Math.floor(entity.y / TILE); const centerX = tileX * TILE + TILE / 2, centerY = tileY * TILE + TILE / 2; if (Math.abs(entity.x - centerX) > turnTolerance || Math.abs(entity.y - centerY) > turnTolerance) return; const [dx, dy] = dirs[entity.want]; if (!walkable(centerX + dx * TILE, centerY + dy * TILE)) return; entity.x = centerX; entity.y = centerY; entity.dir = entity.want; }
function move(entity: { x: number; y: number; dir: Dir; want: Dir }, speed: number, halfWidth: number, halfHeight: number): void { const distance = speed / 20; applyQueuedTurn(entity, TURN_GRACE_DISTANCE); if (entity.dir === "none") return; const [dx, dy] = dirs[entity.dir]; const steps = Math.max(1, Math.ceil(distance / 2)); const step = distance / steps; for (let i = 0; i < steps; i++) { const x = entity.x + dx * step, y = entity.y + dy * step; if (!walkable(x, y)) { if (dx !== 0) entity.x = Math.floor(entity.x / TILE) * TILE + TILE / 2; if (dy !== 0) entity.y = Math.floor(entity.y / TILE) * TILE + TILE / 2; entity.dir = "none"; return; } entity.x = x; entity.y = y; } void halfWidth; void halfHeight; }
...
function spawnGoodShow(room: Room, scheduleNext = true): void { const spaces: string[] = []; const used = new Set(room.goodShows.map((item) => `${Math.floor(item.x / TILE)},${Math.floor(item.y / TILE)}`)); const cactusTiles = [...room.players.values()].map((player) => `${Math.floor(player.x / TILE)},${Math.floor(player.y / TILE)}`); for (let y = 1; y < ROWS - 1; y++) for (let x = 1; x < COLS - 1; x++) { const px = x * TILE + 16; const py = y * TILE + 16; const key = `${x},${y}`; if (walkable(px, py) && !used.has(key) && !cactusTiles.includes(key) && Math.hypot(px - room.coin.x, py - room.coin.y) > 64) spaces.push(key); } if (spaces.length) { const [x, y] = spaces[Math.floor(Math.random() * spaces.length)].split(",").map(Number); room.goodShows.push({ x: x * TILE + 16, y: y * TILE + 16 }); } if (scheduleNext) room.nextGoodShowAt = Date.now() + GOOD_SHOW_INTERVAL; }
function spawnInitialGoodShows(room: Room): void { for (let i = 0; i < INITIAL_GOOD_SHOWS; i++) spawnGoodShow(room, false); }
function tick(room: Room): void { if (room.winner) return; const now = Date.now(); if (now >= room.nextGoodShowAt) spawnGoodShow(room); const coinInput = room.inputs.get("coin"); if (coinInput && now - coinInput.at <= TURN_BUFFER_MS) room.coin.want = coinInput.dir; else if (room.coin.want !== room.coin.dir) room.coin.want = room.coin.dir; move(room.coin, COIN_SPEED, 7, 8); for (const player of room.players.values()) if (player.role === "cactus" && player.jailedUntil <= now) { if (player.x === 240 && player.y === 336) { player.x = 272; player.y = 240; } const input = room.inputs.get(player.id); if (input && now - input.at <= TURN_BUFFER_MS) player.want = input.dir; else if (player.want !== player.dir) player.want = player.dir; move(player, CACTUS_SPEED, 6, 8); }
