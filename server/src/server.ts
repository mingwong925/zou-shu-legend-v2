...
function tick(room: Room): void { if (room.winner) return; const now = Date.now(); if (now >= room.nextGoodShowAt) spawnGoodShow(room); room.coin.want = room.inputs.get("coin") ?? room.coin.want; move(room.coin, COIN_SPEED, 9, 9); for (const player of room.players.values()) if (player.role === "cactus" && player.jailedUntil <= now) { if (player.x === 240 && player.y === 336) { player.x = 272; player.y = 240; } player.want = room.inputs.get(player.id) ?? player.want; move(player, CACTUS_SPEED, 8, 10); }
