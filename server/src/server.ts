...
type Room = { code: string; players: Map<string, Player>; inputs: Map<string, Dir>; dots: Set<string>; coin: { x: number; y: number; dir: Dir; lives: number }; winner: Winner; timer?: NodeJS.Timeout };
...
function newRoom(code: string): Room { return { code, players: new Map(), inputs: new Map(), dots: dots(), coin: { x: 112, y: 624, dir: "none", lives: 3 }, winner: null }; }
...
function snapshot(room: Room) { return { coin: room.coin, cacti: [...room.players.values()].filter((p) => p.role === "cactus").map((p) => ({ id: p.id, x: p.x, y: p.y, dir: p.dir, color: p.color, playerIndex: p.playerIndex })), dots: [...room.dots], winner: room.winner }; }
function tick(room: Room): void { if (room.winner) return; room.coin.dir = room.inputs.get("coin") ?? room.coin.dir; move(room.coin, COIN_SPEED); for (const player of room.players.values()) if (player.role === "cactus") { player.dir = room.inputs.get(player.id) ?? player.dir; move(player, CACTUS_SPEED); }
  const tile = `${Math.floor(room.coin.x / TILE)},${Math.floor(room.coin.y / TILE)}`; room.dots.delete(tile);
  for (const player of room.players.values()) if (player.role === "cactus" && Math.hypot(player.x - room.coin.x, player.y - room.coin.y) < 21) { room.coin.lives--; room.coin.x = 112; room.coin.y = 624; room.coin.dir = "none"; if (room.coin.lives <= 0) room.winner = "cacti"; }
