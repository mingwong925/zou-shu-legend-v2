...
type Player = { id: string; role: "host" | "cactus"; playerIndex: number; color: string; x: number; y: number; dir: Dir; jailedUntil: number };
type Room = { code: string; players: Map<string, Player>; inputs: Map<string, Dir>; dots: Set<string>; coin: { x: number; y: number; dir: Dir; lives: number }; goodShows: { x: number; y: number }[]; nextGoodShowAt: number; frightUntil: number; message: string | null; messageUntil: number; winner: Winner; timer?: NodeJS.Timeout };
...
function newRoom(code: string): Room { return { code, players: new Map(), inputs: new Map(), dots: dots(), coin: { x: 112, y: 624, dir: "none", lives: 3 }, goodShows: [], nextGoodShowAt: Date.now() + GOOD_SHOW_INTERVAL, frightUntil: 0, message: null, messageUntil: 0, winner: null }; }
...
function move(entity: { x: number; y: number; dir: Dir }, speed: number, halfWidth: number, halfHeight: number): void { if (entity.dir === "none") return; const [dx, dy] = dirs[entity.dir]; const distance = speed / 20; const steps = Math.max(1, Math.ceil(distance / 2)); const step = distance / steps; for (let i = 0; i < steps; i++) { const x = entity.x + dx * step, y = entity.y + dy * step; if (!walkableFootprint(x, y, halfWidth, halfHeight)) { entity.dir = "none"; return; } entity.x = x; entity.y = y; } }
...
function tick(room: Room): void { if (room.winner) return; const now = Date.now(); if (now >= room.nextGoodShowAt) spawnGoodShow(room); room.coin.dir = room.inputs.get("coin") ?? room.coin.dir; move(room.coin, COIN_SPEED, 12, 12); for (const player of room.players.values()) if (player.role === "cactus" && player.jailedUntil <= now) { if (player.x === 240 && player.y === 336) { player.x = 272; player.y = 240; } player.dir = room.inputs.get(player.id) ?? player.dir; move(player, CACTUS_SPEED, 13, 19); }
