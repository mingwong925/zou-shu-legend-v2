...
function spawnGoodShow(room: Room, scheduleNext = true): void { const spaces: string[] = []; const used = new Set(room.goodShows.map((item) => `${Math.floor(item.x / TILE)},${Math.floor(item.y / TILE)}`)); for (let y = 1; y < ROWS - 1; y++) for (let x = 1; x < COLS - 1; x++) { const px = x * TILE + 16; const py = y * TILE + 16; if (walkable(px, py) && !room.dots.has(`${x},${y}`) && !used.has(`${x},${y}`) && Math.hypot(px - room.coin.x, py - room.coin.y) > 64) spaces.push(`${x},${y}`); } if (spaces.length) { const [x, y] = spaces[Math.floor(Math.random() * spaces.length)].split(",").map(Number); room.goodShows.push({ x: x * TILE + 16, y: y * TILE + 16 }); } if (scheduleNext) room.nextGoodShowAt = Date.now() + GOOD_SHOW_INTERVAL; }
