...
function applyQueuedTurn(entity: { x: number; y: number; dir: Dir; want: Dir }, halfWidth: number, halfHeight: number): void { if (entity.want === "none") return; const tileX = Math.floor(entity.x / TILE), tileY = Math.floor(entity.y / TILE); const centerX = tileX * TILE + TILE / 2, centerY = tileY * TILE + TILE / 2; if (Math.abs(entity.x - centerX) > 2 || Math.abs(entity.y - centerY) > 2) return; const [dx, dy] = dirs[entity.want]; if (!walkableFootprint(centerX + dx * TILE, centerY + dy * TILE, halfWidth, halfHeight)) return; entity.x = centerX; entity.y = centerY; entity.dir = entity.want; }
function move(entity: { x: number; y: number; dir: Dir; want: Dir }, speed: number, halfWidth: number, halfHeight: number): void { applyQueuedTurn(entity, halfWidth, halfHeight); if (entity.dir === "none") return; const [dx, dy] = dirs[entity.dir]; const distance = speed / 20; const steps = Math.max(1, Math.ceil(distance / 2)); const step = distance / steps; for (let i = 0; i < steps; i++) { const x = entity.x + dx * step, y = entity.y + dy * step; if (!walkableFootprint(x, y, halfWidth, halfHeight)) { entity.dir = "none"; return; } entity.x = x; entity.y = y; applyQueuedTurn(entity, halfWidth, halfHeight); } }
...
function spawnInitialGoodShows(room: Room): void { for (let i = 0; i < INITIAL_GOOD_SHOWS; i++) spawnGoodShow(room); }
 function tick(room: Room): void { if (room.winner) return; const now = Date.now(); if (now >= room.nextGoodShowAt) spawnGoodShow(room); room.coin.want = room.inputs.get("coin") ?? room.coin.want; move(room.coin, COIN_SPEED, 10, 10); for (const player of room.players.values()) if (player.role === "cactus" && player.jailedUntil <= now) { if (player.x === 240 && player.y === 336) { player.x = 272; player.y = 240; } player.want = room.inputs.get(player.id) ?? player.want; move(player, CACTUS_SPEED, 10, 13); }
